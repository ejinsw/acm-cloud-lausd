...
document.addEventListener('DOMContentLoaded', () => {
        ...
        ws.onmessage = (event) => {
            ...
            switch (data.type) {
                case 'REQUEST_USER_INFO':
                    // This is handled by the UI prompting the user.
                    break;
                ...
                case 'NEW_MESSAGE':
                    if (currentRoom && currentRoom.id === data.payload.roomId) {
                        ...
                        appendMessage(data.payload);           // data.payload is the message
                    }
                    break;
                case 'USER_LEFT':
                    if (currentRoom && currentRoom.id === data.payload.roomId) {
    ...
    identifyBtn.addEventListener('click', () => {
        ...
        if (userId && username) {
            if (ws && ws.readyState === WebSocket.OPEN) {
                ...
                alert("Not connected yet. Please wait for connection.");
            }
        } else {
    ...
    createRoomBtn.addEventListener('click', () => {
        ...
        if (roomName && currentUser) {
            sendToServer('CREATE_ROOM', { roomName, user: currentUser });
    ...
    function updateAvailableRooms(rooms) {
        ...
        rooms.forEach(room => {
            ...
            li.textContent = `${room.name} (${room.userCount || 0} users)`; // Handle undefined userCount
            const joinBtn = document.createElement('button');
            joinBtn.textContent = 'Join';
            joinBtn.onclick = () => {
                if (currentUser) {
                    sendToServer('JOIN_ROOM', { roomId: room.id, user: currentUser });
                } else {
    ...
    leaveRoomBtn.addEventListener('click', () => {
        if (currentRoom && currentUser) {
            ...
            // UI update will be handled by YOU_LEFT_ROOM or error
        }
    });


    function appendMessage(message) {
        // Defensive check for message and its properties
        ...
        // Check if currentUser exists and has an id before comparing
